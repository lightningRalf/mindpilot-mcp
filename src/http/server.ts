...
export class SingletonHTTPServer {
  ...
  private async setupRoutes() {
    ...
    if (isProduction) {
      ...
      try {
        ...
        logger.warn("Built client not found. Run 'npm run build' to build the client.");
      }
    }
    ...
    this.fastify.get("/api/status", async (request: FastifyRequest, reply: FastifyReply) => {
      const mcpActiveRecently = (Date.now() - this.lastMcpActivity.getTime()) < this.MCP_TIMEOUT_MS;
      const secondsUntilShutdown = mcpActiveRecently ? null : 
        Math.max(0, 60 - Math.floor((Date.now() - this.lastMcpActivity.getTime()) / 1000));
      
      return reply.send({
        serverRunning: true,
        browserConnections: this.browserConnections.size,
        mcpActive: mcpActiveRecently,
        lastMcpActivitySecondsAgo: Math.floor((Date.now() - this.lastMcpActivity.getTime()) / 1000),
    ...
    this.fastify.post("/api/keepalive", async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        socket.socket.on("message", async (data) => {
          try {
            ...
            switch (message.type) {
              ...
              // Browsers are read-only - they don't render or validate
              default:
                logger.debug('Ignoring message type from browser', { type: message.type });
            }
          } catch (error) {
  ...
  private checkForShutdown() {
    ...
    const mcpInactive = (Date.now() - this.lastMcpActivity.getTime()) >= this.MCP_TIMEOUT_MS;
    
    if (mcpInactive) {
      logger.info(`No recent MCP activity. Server will shut down in ${this.SHUTDOWN_DELAY_MS / 1000} seconds...`);
      
      this.shutdownTimer = setTimeout(() => {
        // Double-check MCP activity before shutdown
        const stillMcpInactive = (Date.now() - this.lastMcpActivity.getTime()) >= this.MCP_TIMEOUT_MS;
